...
import { ArgsChangeRange, RangeSelectorFareProps } from '@vna-base/screens/flight/type';
import { bs, createStyleSheet, useStyles } from '@theme';
import { abbreviationFare, onCheckType, scale } from '@vna-base/utils';
import {
  sharedClamp,
  sharedMax,
  sharedSub,
...
export const RangeSelectorFare = forwardRef(
  (
    { onChangeRange, initialRange, fare, customFee }: RangeSelectorFareProps,
    ref,
  ) => {
    ...
    const range = useMemo(() => fare.max - fare.min, [fare.max, fare.min]);
    ...
    const calText = useCallback(
      ({ value1, value2 }: { value1: number; value2: number }) => {
        valueRef.current?.setNativeProps({
          text: `${abbreviationFare(
            fare.min + customFee + (range * value1) / 100,
          )} - ${abbreviationFare(
            fare.min + customFee + (range * value2) / 100,
          )}`,
        });
      },
      [range],
      ...
      <View style={[bs.paddingVertical_16, bs.rowGap_8]}>
        <View style={styles.titleContainer}>
          ...
          <View style={styles.valueContainer}>
            <TextInput
              ...
              defaultValue="0 - 0"
            />
            <Text text="VND" colorTheme="neutral80" />
          </View>
        </View>
        <View style={styles.barContainer} onLayout={_onLayout}>
        ...
        <View style={styles.maxMinContainer}>
          <Text
            ...
            text={abbreviationFare(fare.min + customFee)}
          />
          <Text
            fontStyle="Body12Reg"
            colorTheme="neutral90"
            text={abbreviationFare(fare.max + customFee)}
          />
        </View>
      </View>
    );
