...
import { createPaymentHistoryInListFromAxios } from '@vna-base/utils/realm/payment-history';
...
export const runPaymentListener = () => {
  takeLatestListeners()({
    actionCreator: paymentActions.getListPaymentExpense,
    effect: async (action, listenerApi) => {
      const { id } = action.payload;

      const response = await Data.paymentPaymentExpenseByOrderCreate({
        Id: id,
        Forced: true,
      });
      ...
      if (validResponse(response)) {
        listenerApi.dispatch(
          paymentActions.saveListPaymentExpense(
            response.data.List as Array<Payment>,
          ),
        );
      }
    },
  ...
  takeLatestListeners()({
    actionCreator: paymentActions.getListPaymentReceive,
    effect: async (action, listenerApi) => {
      const { id } = action.payload;

      const response = await Data.paymentPaymentReceiveByOrderCreate({
        Id: id,
        Forced: true,
      });
      ...
      if (validResponse(response)) {
        listenerApi.dispatch(
          paymentActions.saveListPaymentReceive(
            response.data.List as Array<Payment>,
          ),
        );
      }
  ...
  takeLatestListeners()({
    actionCreator: paymentActions.getListPaymentHistory,
    effect: async (action, listenerApi) => {
      ...
      if (!pageIndex) {
        ...
        listenerApi.dispatch(
          paymentActions.saveResultFilter({
            list: [],
            pageIndex: 1,
            totalPage: 1,
          }),
        ...
        listenerApi.dispatch(paymentActions.changeLoadingFilter(true));
      }

      let _filterForm = filterForm;

      if (!isEmpty(_filterForm)) {
        listenerApi.dispatch(paymentActions.savedFilterForm(_filterForm));
      } else {
        _filterForm = listenerApi.getState().payment.filterForm!;
      }

      const response = await Data.paymentPaymentGetByAgentCreate({
      ...
      if (validResponse(response)) {
        const { List: newPayments, TotalPage } = response.data;
        const listId: Array<string | null> = cloneDeep(
          listenerApi.getState().payment.resultFilter.list,
        );
        ...
        realmRef.current?.write(() => {
          newPayments?.forEach(payment => {
            if (
              ...
              createPaymentHistoryInListFromAxios(payment);

              listId.push(payment.Id!);
            } else {
              // bị trùng thì update data trong realm và xoá id trong listId, push vào cuối listId
              createPaymentHistoryInListFromAxios(payment, UpdateMode.Modified);

              const i = listId.findIndex(id => id === payment.Id);
              listId[i] = null;
      ...
      if (!pageIndex) {
        listenerApi.dispatch(paymentActions.changeLoadingFilter(false));
      }
    },
  });
  ...
  takeLatestListeners(true, {
    ...
    actionCreator: paymentActions.exportExcel,
