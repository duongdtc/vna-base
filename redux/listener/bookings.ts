...
import { takeLatestListeners, takeMultiListeners } from '@vna-base/utils/redux/listener';
        ...
        response.data.List?.map(booking => {
          return {
            ...
            BookingStatus: translate(
              BookingStatusDetails[booking.BookingStatus as BookingStatus].t18n,
            ), // trạng thái
...
takeMultiListeners()({
  ...
  effect: async (action, listenerApi) => {
    ...
    if (isSaveLoading) {
      listenerApi.dispatch(bookingActions.saveLoadingBooking({ [id!]: true }));
    }
    ...
    if (isSaveLoading) {
      listenerApi.dispatch(bookingActions.saveLoadingBooking({ [id!]: false }));
    }
  },
});
